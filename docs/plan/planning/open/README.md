...
| [`048-dcheck-matrix-modul.md`](048-dcheck-matrix-modul.md) | d-check-`matrix`-Modul (Referenzrichtungs-Gate, SDP) — **Resolved 2026-06-17**: Option A (Voll-SDP) via Slice [`049`](../in-progress/049-sdp-matrix-doku-umbau.md); `matrix` aktiv, Spec-Straten zeitlos, `make docs-check` gruen | **Resolved** (Doc-Archiv mit M8-Closure) |
| [`050-dcheck-matrix-supersede-lineage.md`](050-dcheck-matrix-supersede-lineage.md) | d-check-`matrix`-Supersede-Lineage-Carve-out (CR, Folge aus Slice 049): legitime ADR→ADR-Lineage (abloesende ADR verweist auf abgeloeste) wird als `matrix-inactive` geflaggt; `d-check:ignore` greift nicht fuer `matrix`. Workaround: Inline-Code statt Link | d-check-Release mit `allow-supersede-lineage`-Option |
