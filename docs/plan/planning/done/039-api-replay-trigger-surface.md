...
**Status:** **Abgeschlossen (`done/`, 2026-06-18)** — **Phase A geliefert 2026-06-17** ([`ADR 0068`](../../adr/0068-api-replay-binding-persistence.md): `POST /runs` `replay_of`-Feld + persistente `RunMetadata.replay_of`-Spalte (Migration `0003_add_replay_of`) + 422-`reference_run_not_found`-Reject + Response-/Detail-Exposure). **Phase B geliefert 2026-06-18** (Multi-Run-Execution S4, [`ADR 0069`](../../adr/0069-multi-run-execution-and-scenario-store.md) `Accepted`, [Slice-Plan](multi-run-execution-path.md)): `build_run_driver` verdrahtet das persistierte `replay_of` → `finalize()` difft gegen den Referenzlauf (geteilter Sink, keyed by `run_id`). Replay-Paar 039+040 mit-geschlossen; `make gates`/`make docs-check`/`make fullbuild` gruen.
**Datum:** 2026-06-09
