...
D-6 final nach C0-Review-Folge gegen 4 Findings (Audit-Form
Doku + Smokes / SAFE-005 ✓ produktiv per Lastenheft-Traceability
Z. 2291 — `BatteryDevice.apply_command`/Sicherheitsgrenzen +
Power-Clamp / SAFE-006 ✓ produktiv per Lastenheft-Traceability
Z. 2292 — `diff_replay` mit `volatile_fields`/`tick`/`path`/
`classification` / Demo-Compose `ports`-Klausel mit
`GRID_GYM_DEMO_HOST_BIND`-ENV-Override-Default `127.0.0.1`
fuer `GG-DEPLOY-011` / Hardening-Scope eng auf Port-Bind /
kein NEU ADR; kein NEU Trigger). C2/C3 + Self-Close-Folge
C4a/C4b ausstehend. **M6-Welle-5b abgeschlossen 2026-06-07** mit Stack
...
| 5c | SOLLTE-Items + IP/Netz (`GG-SAFE-005/006` + Demo-Compose) | **In Progress 2026-06-07** (C0 dieser Commit) | [`M6-welle-5c.md`](M6-welle-5c.md) | `GG-SAFE-005` ✓ produktiv per Lastenheft-Traceability Z. 2291 (`BatteryDevice.apply_command`/Sicherheitsgrenzen + Power-Clamp + Alarm-Emission an Battery/Load/GridConnection) + `GG-SAFE-006` ✓ produktiv per Lastenheft-Traceability Z. 2292 (`hexagon/core/replay/diff.py::diff_replay` mit `volatile_fields`/`ReplayDelta.tick`/`path`/`ReplayDeltaClassification`) — Audit-Doku verankert; `deploy/compose.yml` `api`-Service `ports`-Klausel auf `GRID_GYM_DEMO_HOST_BIND`-ENV-Override mit Default `127.0.0.1` fuer `GG-DEPLOY-011`-Akzeptanz; Welle-5c-D-1..D-6 final | — (C1 entfaellt; Welle-5c-D-6 schliesst ADR-Bedarf negativ aus) |
