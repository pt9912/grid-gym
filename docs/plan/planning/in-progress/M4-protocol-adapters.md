...
kein `next/`-Zwischenschritt — M3-Welle-7-Closure hatte M4
...
(8 Checkboxen). Welle 7 schliesst M4 in
`done/M4-protocol-adapters.md` ab.
...
3. **Pro Adapter ein Adapter-Smoke-Test via testcontainers**:
   Pattern analog Welle 6c
   ...
   OPC-UA-Server-Container fuer OPC-UA.
4. **`AC-ADAPTER-LIGHTWEIGHT` greift fuer alle
   ...
   Welle 1 ergaenzt den Pfad-Filter um `protocol_*`, falls
   noetig.
...
**Welle-0-Gate-Erwartung:** kein Default-Gate-Sprung; reines
Doc-Arbeitspaket. `make docs-check` cache-frei gruen
...
- ADR-Folge (geplant **erster M4-ADR**) fuer
  ...
  - Decision 2 (Sync vs. async Vertrag): sync-`Protocol`
    mit Adapter-internem Thread/Queue **oder**
    async-`Protocol` mit TickLoop-Shim — ADR setzt die
    Antwort scharf.
  - Decision 3 (Lifecycle): `start`/`stop` bei Service-Boot
    in `bootstrap` **oder** bei `TickLoop.run()`-Start —
    ADR setzt die Antwort scharf.
  - Decision 7 (Snapshot-Pflicht): stateless aus
    Replay-Sicht als Default (Reconnect-State volatile);
    ADR dokumentiert die Reversibilitaet (Schema-Bump
    folgt ADR-0015-Pattern, falls spaeter noetig).
...
- `tools/arch_check.py`-Vorbereitung fuer
  `AC-ADAPTER-LIGHTWEIGHT`-Pfad-Filter um `protocol_*`
  ergaenzen (Decision 6), **bevor** Welle 2 den ersten
  Adapter liefert.
...
- Vorgabe: **bevorzugt dokumentierter Verzicht** mit klarer
  Begruendung (Lizenz/Maintenance-Last der
  `pydnp3`/`asyncio-iec61850`-Bibliotheken; Test-Sibling-
  Container schwer verfuegbar). Alternativ **sehr kleiner
  Spike** (nur Read-Pfad, ein Profil), abhaengig von der
  Bibliotheks-/Testcontainer-Lage **zum Welle-5-Zeitpunkt**.
...
- **`AC-ADAPTER-LIGHTWEIGHT` bricht spaet**: wenn ein
  ...
  Test erst in Welle 2+ an. *Fallback*: Welle 1 erweitert
  den Architektur-Test **vor** dem ersten Adapter-Commit;
  Welle 2 darf nicht mit rotem `AC-ADAPTER-LIGHTWEIGHT`
  starten.
...
| MQTT-Adapter + Topic-Mapping                          | `make test-integration` mit Mosquitto-Smoke (Welle 2) |
| Modbus-Adapter + Register-Mapping + R/W-Smoke         | `make test-integration` mit Modbus-Smoke (Welle 3) |
| OPC-UA-Adapter + Node-ID-Schema + R/W-Smoke           | `make test-integration` mit OPC-UA-Smoke (Welle 4) |
...
| Alle M4-ADRs `Accepted`                               | `docs/plan/adr/00NN-*.md` `Accepted` (Welle 7) |
| End-to-End-Sweep S-1..S-6                             | `done/M4-results.md §4` mit Per-S-Item-Belegen (Welle 7) |
