...
| `028-tick-loop-private-error-import-contract.md` | Slice 027 Review-Folge L-5: neuer arch_check-Contract `AC-TICK-LOOP-PRIVATE-RESUME-ERRORS` (13. arch_check-Contract) gegen modul-lokale `_*`-Imports aus `tick_loop.py` ausserhalb des Moduls. Stand 2026-05-25: `In Progress` — Move `open/` → `in-progress/` (`907c26e`), Scope-Entscheidung Option 2 generisch (Modul-Ebene, nicht nur die 10 Error-Klassen). |
