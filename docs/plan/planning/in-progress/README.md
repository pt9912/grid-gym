...
| `welle-4a.md`             | M3-Welle-4a-Slice-Begleit-Dokument (Multi-Agent-Foundation-Plumbing: ADR 0026 + TickLoop-Registry-API + Schritt-A0-Pre-Tick-Drain + `AgentMessageBus.consume_for` + `_attach_agents()`-Lifecycle). Stand 2026-05-21: `In Progress` (Pre-C0 `a24f733` Rename, C0 dieser Slice-Doc). Sub-Slicing-Vorbereitung fuer Welle 4b (RuleBasedAgent + Scenario-Schema). |
| `M3-faults-agents-observability.md` | M3-Slice-Plan: Fault-Injection + Multi-Agent-Bus + Observability (Log/Metrics/Trace). Stand 2026-05-21: `In Progress` — Welle 0/1/2/3 abgeschlossen, Welle 4 als naechster Schritt mit Sub-Slicing 4a (Foundation-Plumbing) + 4b (RuleBasedAgent + Scenario-Schema). |
