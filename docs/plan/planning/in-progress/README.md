...
| [`M6-welle-4a.md`](M6-welle-4a.md)      | M6-Welle-4a-Slice-Doc (Generated-Trivyignore-Permit; ADR-0011-Schaerfung an ADR-0043 §2.2; vulnignore-Pattern-Import aus m-trace; Trigger-033-Temp-Deferral fuer CVE-2026-42504). — **Done 2026-06-06** mit Stack `9bb6a92..<C3>` (C0/C1 `94dff9e` NEU ADR-0044 `Provisional` / C2 `8fbd17c` NEU `tools/render_trivyignore.py` + `deploy/security/vulnignore.yaml` + Makefile-Hook / C3 dieser Commit). Welle-4-Sub-Slicing-Beschluss (4a Vulnignore + 4b Performance-Bench) per Welle-4a-D-1. C4a/C4b ausstehend als Welle-4b-Pre-C0a/Pre-C0b. |
...
`9bb6a92..<C3>` (NEU ADR-0044 + `tools/render_trivyignore.py`
+ `deploy/security/vulnignore.yaml`; `make fullbuild` cache-
frei gruen via generierter `.trivyignore`; Trigger 033 bleibt
OFFEN als Stable-Watch).
