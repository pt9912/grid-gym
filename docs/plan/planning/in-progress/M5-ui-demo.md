...
**Welle 6b — UI-Visualization (Plan-Items, TBD im Welle-
6b-C0-Slice-Doc):**
...
- **GG-UI-006 (Geraete-Grafik)**: mindestens MVP-
  ...
  inline-SVG-Pattern. Vermutlich NEU
  `GET /runs/{id}/devices`-Endpoint als API-Surface
  (Welle-6b-C0-Pre-Research-Material).
- **GG-UI-008 (Simulationszustaende)**: Dashboard-
  Page mit Laufstatus + Sim-Zeit + Tick-Zaehler +
  Dienst-Zustand. Welle-4a-`/status`-Endpoint hat
  schon alle Pflicht-Felder; Welle-6b-Substanz ist
  reine UI-Page mit HTMX-Polling.
