...
**Aktiver/geplanter Arc (ab 2026-07-15): Spec-Schichtung — [`ADR 0080`](../../adr/0080-three-layer-spec-model.md) Dreischicht-Modell** (`Accepted`; §4-Detailentscheidungen mit Owner ratifiziert, `9e62ee2`). Zieht die fehlende mittlere **Spezifikations-Schicht** (Datei spezifikation.md, „Pflichtenheft") zwischen Vertrag und Architektur ein — Wurzel-Fix der §18/§27.1-Redundanzen (die per-Tabelle „Bezug"-Spalten ↔ traceability.md §27.1-Spiegelung). **Arc gestartet — 083 umgesetzt (2026-07-16):** [`083`](../done/083-spezifikation-layer-discipline-core-move.md) (Fundament `spec/spezifikation.md` + PRINC/CC/SEED-Umzug aus dem Vertrag, atomarer Cut) **abgeschlossen → `done/`**, ebenso [`084`](../done/084-architecture-bezug-drift-fix.md) (Bezug-Drift-Fix ARCH-007/008 + SCN-006) **→ `done/`**; [`085`](../done/085-spezifikation-layer-qs-families-move.md) (QS-Familien, 30 IDs) + [`086`](../done/086-traceability-derived-27-1-finalization.md) (Traceability-Finalisierung: Vertrags-Amendment + §27.1 gegen architecture.md gegroundet + ARCH-006-Fix) **→ `done/`**. **Der Arc ist KOMPLETT** (083/084/085/086 + 087/§19). Ratifizierte Vorgaben ([`ADR 0080`](../../adr/0080-three-layer-spec-model.md) §4): protocol_profiles → Geschwister; Abnahme-Familien (QA/QG/COV/TESTTYPE/ARCHTEST) + SEED → Spezifikation (Praefixe **bleiben**, nur `ids`-Ziel umbiegen); §27.1 authored→derived (**Gate-Stufe 2026-07-17 gestrichen**, Generator vorgezogen — §4.4-4-Amendment). (§4.5-`GG-SPEC-OPEN-*`-Vorgabe **zurückgenommen** 2026-07-16: Spec beschreibt das Soll → keine Offene-Punkte-Sektion in `spezifikation.md`; offene Tooling-Punkte in Planung/086. Das Prinzip ist als [`ADR 0081`](../../adr/0081-open-points-belong-in-planning.md) kodifiziert [Soll-Specs frei von offenen Punkten; offen→Planung, Entscheidung→ADR, Ergebnis→Spec]; `architecture.md`-§19 folgt als dessen erster Slice [`087`](../done/087-architecture-open-points-to-planning.md).) **Zwei Prerequisites je Slice:** Residuum-Umzug in die Spezifikation + Bezug-Spalten-Drift beheben (ARCH-007/008 + SCN-006-Luecke). **Achtung** (ADR §3): ~30 gate-blinde Referenzen in src/tests/.github/pyproject/Makefile → manueller Grep-Sweep. **Arc ABGESCHLOSSEN (2026-07-16):** der Vertrag ist anforderungsrein, jede Umzugs-Familie hat eine autoritative Heimat (spezifikation.md), die Schicht-Struktur zur Aufloesung der §18/Bezug↔§27.1-Redundanz steht. Offen bleibt die **maschinelle** §27.1-Ausbaustufe `authored → derived` als [`Trigger 088`](../open/088-27-1-consistency-gate-generator.md); das Gate ist gestrichen (§4.4-4-Amendment 2026-07-17), Bezug-Reinigung + Anmerkungs-Umzug sind repo-lokal schneidbar, nur der Generator wartet cross-repo. **Gesamter Arc 083–086 = doku-/config-/kommentar-only → kein Release ueber alle vier Slices.**
**Stand:** 2026-07-16
...
**Kein aktiver Slice.** Der Spec-Schichtungs-Arc ([`ADR 0080`](../../adr/0080-three-layer-spec-model.md), s. Kopf-Status) ist **abgeschlossen + Closure-Review-gehärtet** (2026-07-16, `d8d810c`). Einziger offener Arc-Faden: die §27.1-Ausbaustufe `authored → derived` ([`Trigger 088`](../open/088-27-1-consistency-gate-generator.md)). **Stand 2026-07-17 (`43f4c60`, [`ADR 0080`](../../adr/0080-three-layer-spec-model.md) §4.4-4-Amendment):** Das Konsistenz-**Gate** (Stufe iii) ist **gestrichen** — die Messung mit d-check v0.45.1 (161 Differenzen = 86 F\B + 75 B\F, davon 65 B\F legitime Formunterschiede) zeigt, dass §27.1 kein Bezug-Spiegel ist, ein `mode: equal`-Gate also für immer rot bliebe. **Nächster konkreter Slice-Kandidat (repo-lokal, JETZT schneidbar, kein d-check-Feature nötig):** Trigger-088-Punkte (1) Bezug-Reinigung (Arbeitsliste: 85 echte F\B + 8 echte B\F, u. a. der [`GG-DEV-001`](../../../../spec/lastenheft.md#gg-dev-001)`..018`-Phantom-Range) + (2) Kanten-Anmerkungen aus §27.1 in die `Bezug`-Zelle umziehen. Nur der **Generator** (Stufe iv, Punkt 3) wartet noch cross-repo auf d-check. Zuletzt abgeschlossen (2026-07-14): Delivery-Audit
