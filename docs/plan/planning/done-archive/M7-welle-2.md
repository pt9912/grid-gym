...
- **D-2 = A (standalone Replay)** — zwei Headless-Laeufe via
  ...
  und umgeht bewusst den **[Trigger 040](../done/040-replay-finalize-headless-run-end-seam.md)**-
...
[`next/041`](../done/041-adapter-pure-ignore-imports-rueckbau.md) +
[`next/042`](../done/042-fault-engine-location-and-naming.md).
...
| --- | --- | --- |
| Deterministische Replay-Pruefung | Core-Diff `diff_replay()` ✓ produktiv (M6-Welle-5c-Audit); plus optional `ReplaySourcePort`-E2E aus [`M7-welle-1.md`](M7-welle-1.md) (`GG-MVP-002`). | ✗ kein Aggregat-Aufruf |
...
| Demo-Healthcheck | `GET /health` ✓ produktiv (Liveness); `GET /ready` Three-State produktiv nach M6-Welle-6-C2 (siehe [`../done/M6-welle-6.md`](M6-welle-6.md) — aktiv) | ✗ kein Aggregat-Aufruf |
...
- ⚠ `GET /ready` Three-State-Endpoint: produktiv erst nach
  ...
  [`../done/M6-welle-6.md`](M6-welle-6.md)).
