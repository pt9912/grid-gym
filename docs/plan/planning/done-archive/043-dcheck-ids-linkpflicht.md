...
kein Doku-Target).
**Dritter Nachtrag (User-Entscheid, gleicher Tag):** NEU
`planning/done-archive/` als eingefrorene Detail-Historie —
loest den Zukunfts-Gap des pauschalen `done/**`-Ignores (neue
Closure-Artefakte wie `M*-results.md` entstehen direkt in
`done/` und sind jetzt voll in der Linkpflicht; 208 Kennungen
in den results-Docs + done/README verlinkt). Der `ids`-Ignore
bindet an die Archiv-Grenze; `links`/`anchors` pruefen das
Archiv weiter. Endstand der Ausnahmen: NUR
`done-archive/**` + `CHANGELOG.md`.
**Datum:** 2026-06-12
...
- [`../done/002-check-refs-tool.md`](002-check-refs-tool.md)
