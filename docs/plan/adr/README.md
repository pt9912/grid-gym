...
| 0015 | [TickLoop-Snapshot-v2 im SnapshotEnvelope-Pattern (M2 Welle 6a)](0015-snapshot-envelope-v2.md) | Proposed    | 2026-05-19  | Welle-6a-TickLoop-Snapshot-Bump: `TickLoop.snapshot()["version"]` von `1` (M1-Welle-4-Stand) auf `2`; der generische `SnapshotEnvelope`-Konstruktor bleibt tolerant. Strukturierender Bruch — der Inhalts-Vertrag des `sub_snapshots`-Mapping erweitert sich um `devices.<device_type>.<device_id>` x 5 und `grid_model`. v1-Read in M2-Code-Pfaden wirft typisierten `TickLoopSnapshotVersionError`; Backward-Compat-Lese-Migration ist M6 `GG-PERSIST-*`. Unbekannte Sub-Snapshot-Keys werden toleriert (Forward-Compat fuer Welle 7+ `agents.*` o.ae.). Status-Uebergang `Proposed → Provisional` mit Welle-6a-PR, `→ Accepted` mit Welle-6c-Closure. |
