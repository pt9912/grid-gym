...
| 0021 | [Scenario-Loader + TickLoop-Event-Wiring + GridConnection-Auto-Schluss (M2 Welle 6b)](0021-scenario-loader-and-tick-loop-event-wiring.md) | Proposed    | 2026-05-19  | Welle-6b-Builder-/Wiring-ADR: `hexagon/core/scenario/loader.py` erweitert um `build_devices(...)` (Factory-Dispatch nach `ScenarioDevice.type`), `build_tick_loop(...)` (produktiver TickLoop-Builder), und `parse_load_events`/`parse_load_profiles`-Funktionen. TickLoop erhaelt einen Vor-Tick-Block, der LoadProfiles (Baseline) → LoadEvents (Overlay) → GridConnection-Auto-Schluss (`-pre_grid_residual_kw`) auf Devices anwendet. SmartMeter-`attach_sources`-Aufruf ist Scenario-Loader-Verantwortung nach Device-Konstruktion. Neue typed Exceptions `ScenarioUnknownDeviceTypeError` + `ScenarioMissingSourceDeviceError`. Plugin-Registry fuer externe DeviceModel-Klassen bleibt M5/M6-out-of-scope. Status-Uebergang `Proposed → Provisional` mit Welle-6b-PR, `→ Accepted` mit Welle-6c-Closure. |
