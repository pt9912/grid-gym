...
| 0015 | [SnapshotEnvelope-Versions-Bump v1→v2 (M2 Welle 6a)](0015-snapshot-envelope-v2.md) | Proposed    | 2026-05-19  | Welle-6a-Envelope-Bump: `SnapshotEnvelope.version` von `1` (M1-Welle-4-Stand) auf `2`. Strukturierender Bruch — der Inhalts-Vertrag der `sub_snapshots`-Mapping erweitert sich um `devices.<id>` x 5 und `grid_model` (kein additives Schema). v1-Read in M2-Code-Pfaden wirft typisierten `SnapshotEnvelopeSchemaVersionError`; Backward-Compat-Lese-Migration ist M6 `GG-PERSIST-*`. Unbekannte Sub-Snapshot-Keys werden toleriert (Forward-Compat fuer Welle 7+ `agents.*` o.ae.). Status-Uebergang `Proposed → Provisional` mit Welle-6a-PR, `→ Accepted` mit Welle-6c-Closure. |
