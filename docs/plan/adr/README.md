...
| 0019 | [Grid-Model-Bilanz-Pattern (M2 Welle 5a)](0019-grid-model-bilanz-pattern.md) | Provisional | 2026-05-19  | Welle-5a-Modul `hexagon/core/grid_model/` (`bilanz.py` + `config.py` + `snapshot.py`). Proportionales Frequenz/Spannungs-Modell `f = f_nom + k_f * imbalance`, `v = v_nom + k_v * imbalance` mit Safety-Clamps. **Kein DeviceModel** — Single-Instance-System-Modell, `grid_model` als Snapshot-Sub-Key (kein `devices.<id>`). Imbalance-Berechnung schliesst die Welle-5-Netzbilanz aus [`ADR 0016`](0016-pv-load-device-pattern.md) §2.2 und [`ADR 0017`](0017-grid-connection-device-pattern.md) §2.2; GridConnection wird in Welle 6 als Bilanz-Schluss-Variable verdrahtet. §§3/5/6 geschaerft durch 3 Pre-Implementation-Runden + Welle-5a-Review-Folge (Commits `d384218`/`eeeb034`/`a858be8` + `1af57b8`). Status-Uebergang: `Proposed → Provisional` (Welle 5a-Merge) → `Accepted` (Welle-5-Closure mit ADR 0020). |
| 0020 | [Load-Profile + Load-Event Pattern (M2 Welle 5b)](0020-load-profile-and-event-pattern.md) | Proposed    | 2026-05-19  | Welle-5b-Erweiterung von `hexagon/core/grid_model/` um `loads.py` (`LoadEvent` + `LoadProfile` + CSV/JSON-Loader). Schwester-ADR zu [`ADR 0019`](0019-grid-model-bilanz-pattern.md): Welle 5a ist Physik, Welle 5b ist Daten-/I/O-Integration. `GridModelSnapshot` bumpt v1→v2 mit Backward-Compat-Lesepfad (v1-Snapshot ohne LoadEvents/Profiles bleibt roundtrip-faehig). Liefert `GG-GRID-003` (Lasten konstant/Zeitreihe/Event) + `GG-GRID-004` (Lastspruenge). M3-Driftrisiko gegenueber Replay-Source-Pfad explizit dokumentiert (M3 baut auf 5b, repliziert nicht). Status-Uebergang `Proposed → Provisional` mit Welle-5b-PR, `→ Accepted` mit Welle-5-Closure. |
