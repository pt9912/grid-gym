...
| 0015 | [TickLoop-Snapshot-v2 im SnapshotEnvelope-Pattern (M2 Welle 6a)](0015-snapshot-envelope-v2.md) | Provisional | 2026-05-19  | Welle-6a-TickLoop-Snapshot-Bump: `TickLoop.snapshot()["version"]` von `1` (M1-Welle-4-Stand) auf `2`; der generische `SnapshotEnvelope`-Konstruktor bleibt tolerant. Strukturierender Bruch — der Inhalts-Vertrag des `sub_snapshots`-Mapping erweitert sich um `devices.<device_type>.<device_id>` x 5 und `grid_model`. v1-Read in M2-Code-Pfaden wirft typisierten `TickLoopSnapshotVersionError` (mit M6-`GG-PERSIST-*`-Pointer in der Message). Unbekannte Sub-Snapshot-Keys werden toleriert (Forward-Compat fuer Welle 7+ `agents.*` o.ae.). Schreib-Pfad-Exception `TickLoopUnknownDeviceTypeError` fuer fehlende `_DEVICE_TYPE_BY_CLASS_NAME`-Eintraege. §4-Forward-Pointer fuer Welle-7+/M3-`device_type`-Protocol-Property. Status-Uebergang: `Proposed → Provisional` (Welle 6a-Merge) → `Accepted` (Welle-6c-Closure). |
