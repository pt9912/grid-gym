...
| 0022 | [Fault-Injection-Protocol + Scenario-Validator-Härtung (M3 Welle 1)](0022-fault-injection-protocol.md) | Proposed    | 2026-05-20  | M3-Welle-1-Erstwurf: `FaultInjectableDevice(DeviceModel)` Sub-Protocol unter `hexagon/core/faults/_protocol.py` (per ADR 0013 §2.8 — keine Erweiterung der Base-`DeviceModel`-Surface, sondern Closed-Set-Sub-Protocol mit `inject_fault(fault_type, payload)`). Neuer Driven-Port `FaultPort` unter `ports/driven/fault.py` mit `apply_active_faults(devices, context) -> None` (Orchestrierungs-Hook, nicht Per-Device-Hook). Scenario-Validator-Haertung: `_assert_fault_list` prueft Target-Existenz analog `_assert_event_list` (neuer typed `ScenarioUnknownFaultTargetError`); KEINE Type-Whitelist, KEINE semantische Validation (Welle-2-Material). TickLoop-Hook im Vor-Tick-Block nach `_consume_load_inputs_into`, vor erster `_run_device_iteration`; neuer Konstruktor-Kwarg `fault_port: FaultPort \| None = None` (keyword-only, kein produktiver NullFaultPort — Hook skippt bei None). Snapshot-Schema bleibt v2 in Welle 1; Bump v2→v3 wird in Welle 2 oder M6 entschieden. Welle 2 liefert konkrete Adapter (`BatteryFaultAdapter`, `GridFaultAdapter`) + Fault-Typen (`cell_failure`, `voltage_drop`) + Recovery-Logik. Status-Uebergang geplant: `Proposed → Provisional` (Welle-1-Merge) → `Accepted` (M3-Welle-7-Closure). |
