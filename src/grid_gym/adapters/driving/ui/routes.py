"""Page-Routes fuer das UI-Driving-Adapter (M5 Welle 2/3/4a,
ADR 0036 + 0038 + 0039).
...
Welle-4a-Erweiterung (ADR 0039 Decision 14):

- ``GET /runs/{run_id}/control`` — Replay-Controls-Page mit
  Pause/Resume/Stop-Buttons (HTMX-POST gegen `POST /control`)
  + Status-Block mit HTMX-Polling (`hx-trigger="every 1s"`
  auf `GET /status`); erfuellt `GG-UI-004`.
...
Welle 4b ergaenzt die Alarm-Tabelle; Welle 5 den Scenario-
Editor.
"""
...
def get_run_control(
    ...
    """Replay-Controls-Page (M5 Welle 4a, ADR 0039 Decision 14).

    Rendert eine Page mit 3 HTMX-POST-Buttons
    (`pause`/`resume`/`stop`) gegen `POST /runs/{run_id}/control`
    + Status-Polling-Block (`hx-trigger="every 1s"` auf
    `GET /runs/{run_id}/status`). Erfuellt `GG-UI-004`. Bei
    nicht-existentem Run liefert die Route 404 (analog
    Dashboard-Pattern).
    """
    if not repository.exists(run_id):
        raise HTTPException(status_code=404, detail=f"Run '{run_id}' not found.")
