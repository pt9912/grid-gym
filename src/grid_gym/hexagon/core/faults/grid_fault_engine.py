"""`GridFaultEngine` — GridConnection-`voltage_drop`-Faults
...
Klasse fixiert nur `supported_types` auf `voltage_drop` und das
Subsystem-Label.
...
class GridFaultEngine(ScenarioFaultEngine):
    ...
    def __init__(self, faults: Sequence[ScenarioFault]) -> None:
        super().__init__(faults, frozenset({FAULT_TYPE_VOLTAGE_DROP}), "grid_connection")
