...
make test-unit          # unit test suite (1610 tests as of 2026-06-01,
                        # M5-Welle-2 closure)
make test-integration   # Compose/testcontainers integration suite
                        # (43 passed + 4 skipped tests incl. OTLP, MQTT, Modbus,
                        # OPC-UA, DNP3, IEC-61850, M5-HTTP-API and
                        # M5-UI-Foundation smokes)
```
...
- **M5 — UI + Demo** · `In Progress` 2026-06-01
  ...
  - Wave 3 — Live-Telemetry-Dashboard · `Pending` (next active slice)
- **M6 — Performance + Security + CI/CD** · `Pending`

**Test balance:** 1610 unit tests + 43 integration tests passed + 4 skipped
(state after M5 Wave 2 closure 2026-06-01 — +10 unit tests vs. M5
Wave 1 for UI-Foundation: 3 in `test_templates.py` (Jinja2-Factory + path
resolution + render smoke) + 7 in `test_routes.py` (2 page-route full +
2 HTMX-partial paths + 3 static-mount assets); +2 integration tests for
the end-to-end UI-Foundation smoke + OpenAPI-schema check with
